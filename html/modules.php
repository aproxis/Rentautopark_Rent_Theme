...
function modChrome_T3Section($module, &$params, &$attribs) { 
	...
	$moduleClass    = !empty($bootstrapSize) ? ' col-sm-' . (int) $bootstrapSize . '' : '';
	$moduleClassSfx = htmlspecialchars($params->get('moduleclass_sfx')??'');
	$enablTitle			= $params->get('module-specical-title','0');
  ...
  if($bgImage) $moduleClass = $moduleClass.' t3-section-bg'; 
	if (!empty ($module->content)) {
		...
		if(!$enablTitle) {
			if ($module->showtitle != 0 || $moduleIntro) {
				$html .= "<div class=\"section-header {$headerClass}\">";
			}

			if ($module->showtitle != 0) {
				$html .= "<{$headerTag}>{$module->title}</{$headerTag}>";
			}

			if ($moduleIntro) {
				$html .="<p class=\"container-sm module-intro\">{$moduleIntro}</p>";
			}
