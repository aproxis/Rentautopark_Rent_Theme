...
<div class="car-detail">
  <div class="row">
    <div class="col-md-8">
      ...
      if (!empty ($car['img'])) {
        ...
        <div class="vrc-cdetails-cmainimg">
				  <img src="<?php echo JURI::root().$imgpath; ?>" alt="<?php echo Text::_('VRCLISTSFROM'); ?>" />
        </div>
        <?php
      }
      ?>
    ...
    <div class="col-md-4">
      <div class="vrclistcarcat"><span><?php echo vikrentcar::sayCategory($car['idcat'], $vrc_tn); ?></span></div>
      ...
      <div class="vrc-reqinfo-cont">
        <span><a href="Javascript: void(0);" onclick="vrcShowRequestInfo();" class="vrc-reqinfo-opener vrc-pref-color-btn"><?php echo Text::_('VRCCARREQINFOBTN'); ?></a></span>
      </div>
          ...
          <form action="<?php echo JRoute::_('index.php?option=com_vikrentcar&task=reqinfo'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post" onsubmit="return vrcValidateReqInfo();">
            ...
            if (count($this->terms_fields)) {
              if (!empty($this->terms_fields['poplink'])) {
                $fname = "<a href=\"" . $this->terms_fields['poplink'] . "\" id=\"vrcf\" rel=\"{handler: 'iframe', size: {x: 750, y: 600}}\" target=\"_blank\" class=\"vrcmodalframe\">" . Text::_($this->terms_fields['name']) . "</a>";
              } else {
                $fname = "<label id=\"vrcf\" for=\"vrcf-inp\" style=\"display: inline-block;\">" . Text::_($this->terms_fields['name']) . "</label>";
              }
              ?>
              ...
              ?>
              <div class="vrcdialog-reqinfo-formentry vrcdialog-reqinfo-formentry-ckbox">
                <label id="vrcf" for="vrcf-inp" style="display: inline-block;"><?php echo Text::_('ORDER_TERMSCONDITIONS'); ?></label>
                <input type="checkbox" name="vrcf" id="vrcf-inp" value="<?php echo Text::_('VRYES'); ?>"/>
              </div>
              <?php
            }
      ...
      if ($car['cost'] > 0) { ?>
        <div class="vrc-cdetails-cost vrc-pref-color-text">
          <span class="vrcliststartfrom"><?php echo Text::_('VRCLISTSFROM'); ?></span>
...
<div class="vrc-cardetails-legend">
	...
	<div class="vrclegendediv">
		...
		<span class="vrclegenda">
			...
			<span class="vrc-leg-text"><?php echo Text::_('VRLEGFREE'); ?></span>
		</span>
		<?php
		if ($showpartlyres) {
		?>
		<span class="vrclegenda">
			...
			<span class="vrc-leg-text"><?php echo Text::_('VRLEGWARNING'); ?></span>
		</span>
		<?php
		}
		?>
		<span class="vrclegenda">
			...
			<span class="vrc-leg-text"><?php echo Text::_(($show_hourly_cal ? 'VRLEGBUSYCHECKH' : 'VRLEGBUSY')); ?></span>
		</span>
...
?>
	<div class="vrc-hourlycal-container">
		<h4 class="vrc-medium-header"><?php echo Text::sprintf('VRCAVAILSINGLEDAY', date($df, $viewingdayts)); ?></h4>
		<div class="table-responsive">
			<table class="vrc-hourly-cal table">
				<tr>
					<td style="text-align: center;"><?php echo Text::_('VRCLEGH'); ?></td>
<?php
for ($h = 0; $h <= 23; $h++) {
	if ($nowtf == 'H:i') {
		$sayh = $h < 10 ? "0".$h : $h;
...
<h3><?php echo Text::_('VRCSELECTPDDATES'); ?></h3>
<?php
if (VikRentCar::allowRent()) {
	...
	$coordsplaces = array();
	$selform = "<div class=\"vrcdivsearch\"><div class=\"vrcdivsearch-inner\"><form action=\"".JRoute::_('index.php?option=com_vikrentcar'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''))."\" method=\"get\" onsubmit=\"return vrcValidateSearch();\">\n";
...
function pickupClosingDays(date) {
	...
	if (jQuery.inArray(dmy, checklocarr) == -1 && jQuery.inArray(wday, checklocarr) == -1) {
		...
		return [false, "", "'.addslashes(Text::_('VRCLOCDAYCLOSED')).'"];
	}
}
...
jQuery(function() {
	...
	jQuery('.vr-cal-img, .vrc-caltrigger').click(function() {
		...
		$selform .= '		<label for="pickupdate">' . Text::_('VRPICKUPCAR') . '</label>' . "\n";
		...
		if (!strlen($forced_pickup)) {
			...
			$selform .= '		<label>' . Text::_('VRALLE') . '</label>' . "\n";
		...
		$selform .= '<div class="vrcsfentrycont">' . "\n";
		$selform .= '	<div class="vrcsfentrylabsel">' . "\n";
		$selform .= '		<label for="releasedate">' . Text::_('VRRETURNCAR') . '</label>' . "\n";
		$selform .= '		<div class="vrcsfentrydate">' . "\n";
		...
		if (!strlen($forced_dropoff)) {
			$selform .= '	<div class="vrcsfentrytime">' . "\n";
			$selform .= '		<label>' . Text::_('VRALLEDROP') . '</label>' . "\n";
			$selform .= '		<div class="vrc-sf-time-container">' . "\n";
	...
	//
	$selform .= "<div class=\"vrcsfentrycont\"><div class=\"vrcsfentrysubmit\"><input type=\"submit\" name=\"search\" value=\"" . Text::_('VRCBOOKTHISCAR') . "\" class=\"btn vrcdetbooksubmit vrc-pref-color-btn\"/></div></div>\n";
	// close submit part
	$selform .= "</div>\n";
		...
		if (vrc_mopening_pick !== null) {
			...
			if (pickh == vrc_mopening_pick[0]) {
				if (pickm < vrc_mopening_pick[1]) {
					...
					alert(Joomla.Text._('VRC_LOC_WILL_OPEN_TIME').replace('%s', vrcFormatTime(vrc_mopening_pick[0], vrc_mopening_pick[1])));
			...
			if (pickh == vrc_mopening_pick[2]) {
				if (pickm > vrc_mopening_pick[3]) {
					...
					alert(Joomla.Text._('VRC_LOC_WILL_CLOSE_TIME').replace('%s', vrcFormatTime(vrc_mopening_pick[2], vrc_mopening_pick[3])));
					// do not return false as we are overwriting the pickup time
					// return false;
				}
			}
			...
			if (vrc_mopening_pick[4] && vrc_mopening_pick[4].length) {
				for (var b = 0; b < vrc_mopening_pick[4].length; b++) {
					...
					var break_mess = Joomla.Text._('VRC_PICKLOC_IS_ON_BREAK_TIME_FROM_TO')
		...
		if (vrc_mopening_drop !== null) {
			...
			if (droph == vrc_mopening_drop[0]) {
				if (dropm > vrc_mopening_drop[1]) {
					...
					alert(Joomla.Text._('VRC_LOC_WILL_CLOSE_TIME').replace('%s', vrcFormatTime(vrc_mopening_drop[0], vrc_mopening_drop[1])));
			...
			if (droph == vrc_mopening_drop[2]) {
				if (dropm < vrc_mopening_drop[3]) {
					...
					alert(Joomla.Text._('VRC_LOC_WILL_OPEN_TIME').replace('%s', vrcFormatTime(vrc_mopening_drop[2], vrc_mopening_drop[3])));
					// do not return false as we are overwriting the drop off time
					// return false;
				}
			}
			...
			if (vrc_mopening_drop[4] && vrc_mopening_drop[4].length) {
				for (var b = 0; b < vrc_mopening_drop[4].length; b++) {
					...
					var break_mess = Joomla.Text._('VRC_DROPLOC_IS_ON_BREAK_TIME_FROM_TO')
