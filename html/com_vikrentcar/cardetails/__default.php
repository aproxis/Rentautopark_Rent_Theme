...
<?php /* ================================================================
   3. BOOKING CARD - NEW TAILWIND DESIGN
   ================================================================ */ ?>
...
<div class="cd-booking-card">
	...
	<!-- New Tailwind Form Layout -->
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
		<!-- Pickup Section -->
		<div>
			<div class="flex items-center gap-2 mb-2">
				<div class="w-6 h-6 bg-[#FE5001] text-white rounded-full flex items-center justify-center font-bold text-xs">1</div>
				<h4 class="text-sm font-bold text-foreground">Получение</h4>
			</div>
			<div class="space-y-2 pl-6">
				<div class="grid grid-cols-2 gap-2">
					<div>
						<label class="block text-xs font-medium text-gray-700 mb-1">
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-map-pin w-3 h-3 inline mr-1">
								<path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"></path>
								<circle cx="12" cy="10" r="3"></circle>
							</svg>Место получения
						</label>
						<div class="relative">
							<select name="place" id="place" class="w-full pl-3 pr-8 py-[9px] text-xs border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FE5001] focus:border-[#FE5001] appearance-none bg-white cursor-pointer font-medium transition-all">
								<?php
								// Load places for the new form
								$dbo = JFactory::getDbo();
								...
								if ($dbo->getNumRows() > 0) {
									...
									foreach ($places as $pla) {
										if (!in_array($pla['id'], $plapick)) continue;
										echo '<option value="' . $pla['id'] . '" id="place' . $pla['id'] . '">' . $pla['name'] . '</option>';
									}
								}
								?>
							</select>
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-down absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none">
					...
					<div>
						<label class="block text-xs font-medium text-gray-700 mb-1">
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-calendar w-3 h-3 inline mr-1">
								<path d="M8 2v4"></path>
								<path d="M16 2v4"></path>
						...
						<div class="relative">
							<button type="button" class="w-full pl-3 pr-8 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FE5001] appearance-none bg-white cursor-pointer transition-all text-left py-[7px] text-[15px] border border-gray-300 focus:border-transparent" id="pickupdate-btn">
								<span class="font-medium text-[15px] text-gray-400">Выберите</span>
							</button>
				...
				<div class="grid grid-cols-2 gap-2">
					...
					<div>
						<label class="block text-xs font-medium text-gray-700 mb-1">
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-clock w-3 h-3 inline mr-1">
								<circle cx="12" cy="12" r="10"></circle>
								<polyline points="12 6 12 12 16 14"></polyline>
							</svg>Время
						</label>
						<div class="relative">
							<select name="pickuph" id="vrccomselph" class="w-full pl-3 pr-8 py-[9px] text-xs border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FE5001] focus:border-[#FE5001] appearance-none bg-white cursor-pointer font-medium transition-all">
								<?php
								...
								if (!($i < $j)) {
									while (intval($i) != (int)$j) {
										$sayi = $i < 10 ? "0" . $i : $i;
										echo '<option value="' . (int)$i . '">' . $sayi . '</option>';
										$i++; $i = $i > 23 ? 0 : $i;
									}
									$sayi = $i < 10 ? "0" . $i : $i;
									echo '<option value="' . (int)$i . '">' . $sayi . '</option>';
		...
		<div>
			<div class="flex items-center gap-2 mb-2">
				<div class="w-6 h-6 bg-[#FE5001] text-white rounded-full flex items-center justify-center font-bold text-xs">2</div>
				<h4 class="text-sm font-bold text-foreground">Возврат</h4>
			</div>
			<div class="space-y-2 pl-6">
				<div class="grid grid-cols-2 gap-2">
					<div>
						...
						<div class="relative">
							<select name="returnplace" id="returnplace" class="w-full pl-3 pr-8 py-[9px] text-xs border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FE5001] focus:border-[#FE5001] appearance-none bg-white cursor-pointer font-medium transition-all">
								<?php
								if ($dbo->getNumRows() > 0) {
									$pladrop = explode(';', $car['idretplace']);
									foreach ($places as $pla) {
										if (!in_array($pla['id'], $pladrop)) continue;
										echo '<option value="' . $pla['id'] . '" id="returnplace' . $pla['id'] . '">' . $pla['name'] . '</option>';
									}
								}
								?>
							</select>
					...
					<div>
						...
						<div class="relative">
							<button type="button" class="w-full pl-3 pr-8 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FE5001] appearance-none bg-white cursor-pointer transition-all text-left py-[7px] text-[15px] border border-gray-300 focus:border-transparent" id="releasedate-btn">
								<span class="font-medium text-[15px] text-gray-400">Выберите</span>
							</button>
				...
				<div class="grid grid-cols-2 gap-2">
					...
					<div>
						...
						<div class="relative">
							<select name="releaseh" id="vrccomseldh" class="w-full pl-3 pr-8 py-[9px] text-xs border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FE5001] focus:border-[#FE5001] appearance-none bg-white cursor-pointer font-medium transition-all">
								<?php
								// Same hours for return time
								if (!($i < $j)) {
									$i = $opent[0];
									while (intval($i) != (int)$j) {
										$sayi = $i < 10 ? "0" . $i : $i;
	...
	<div class="flex justify-center">
		<div class="w-full max-w-md">
			<input type="submit" name="search" value="<?php echo Text::_('VRCBOOKTHISCAR'); ?>" class="btn vrcdetbooksubmit vrc-pref-color-btn w-full" />
		</div>
	</div>
	...
	<div class="cd-shield-info">
		...
		<span><?php echo Text::_('VRCSHIELDINFO') ?: 'Anulare gratuită cu 72h înainte • Suport 24/7 • Mașini asigurate'; ?></span>
	</div>

	<?php } else { ?>
	<div class="cd-disabled-rent"><?php echo VikRentCar::getDisabledRentMsg(); ?></div>
	<?php } ?>
</div><!-- /.cd-booking-card -->
...
/* New form JavaScript for Tailwind design */
jQuery(function() {
	// Initialize date pickers for button elements
	var $pickupdateBtn = jQuery('#pickupdate-btn');
	var $releasedateBtn = jQuery('#releasedate-btn');
	var $pickupdateHidden = jQuery('#pickupdate');
	var $releasedateHidden = jQuery('#releasedate');
	...
	var datePickerOptions = {
		...
		beforeShowDay: function(date) {
			...
			// Check if date is disabled for pickup
			if (jQuery.inArray('"' + dateStr + '"', <?php echo json_encode($push_disabled_in); ?>) >= 0) {
				return [false, '', ''];
			}
			return [true, '', ''];
		},
		onSelect: function(dateText, inst) {
			...
			if (this.id === 'pickupdate') {
				$pickupdateHidden.val(dateText);
				$pickupdateBtn.find('span').text(dateText).removeClass('text-gray-400').addClass('text-gray-900 font-medium');
				// Trigger location change logic if needed
			...
			} else if (this.id === 'releasedate') {
				$releasedateHidden.val(dateText);
				$releasedateBtn.find('span').text(dateText).removeClass('text-gray-400').addClass('text-gray-900 font-medium');
				if (typeof vrcLocationWopening !== 'undefined') {
	...
	$pickupdateBtn.click(function() {
		$pickupdateHidden.datepicker(datePickerOptions).datepicker('show');
	});
	
	$releasedateBtn.click(function() {
		$releasedateHidden.datepicker(datePickerOptions).datepicker('show');
	});

	// Location change handlers
	jQuery('#place').change(function() {
		var selectedPlace = jQuery(this).val();
		// Trigger existing location change logic
		if (typeof vrcSetLocOpenTime !== 'undefined') {
			vrcSetLocOpenTime(selectedPlace, 'pickup');
		}
	});
	
	jQuery('#returnplace').change(function() {
		var selectedPlace = jQuery(this).val();
		// Trigger existing location change logic
		if (typeof vrcSetLocOpenTime !== 'undefined') {
			vrcSetLocOpenTime(selectedPlace, 'dropoff');
		}
	});
	...
	jQuery('form').on('submit', function() {
		...
		var returnplace = jQuery('#returnplace').val();
		var pickupdate = jQuery('#pickupdate').val();
		var releasedate = jQuery('#releasedate').val();
		
		if (!place || !returnplace || !pickupdate || !releasedate) {
			alert('<?php echo addslashes(Text::_('VRFILLALL')); ?>');
			return false;
		}
		
		// Call existing validation function if available
		if (typeof vrcValidateSearch !== 'undefined') {
			return vrcValidateSearch();
		}
	...
	<?php if (!empty($pday) && $pday >= $nowts): ?>
	var currentDate = new Date(<?php echo date('Y', $viewingdayts); ?>, <?php echo ((int)date('n', $viewingdayts) - 1); ?>, <?php echo date('j', $viewingdayts); ?>);
	$pickupdateHidden.datepicker('setDate', currentDate);
	$pickupdateBtn.find('span').text(jQuery.datepicker.formatDate('yy-mm-dd', currentDate)).removeClass('text-gray-400').addClass('text-gray-900 font-medium');
	<?php endif; ?>
});

/* Override existing VRC functions to work with new form structure */
if (typeof vrcSetMinDropoffDate !== 'undefined') {
	var originalVrcSetMinDropoffDate = vrcSetMinDropoffDate;
	vrcSetMinDropoffDate = function() {
		// Call original function
		originalVrcSetMinDropoffDate();
		
		// Update button text if date is set
		var minDate = jQuery('#releasedate').datepicker('option', 'minDate');
		if (minDate) {
			var formattedDate = jQuery.datepicker.formatDate('yy-mm-dd', minDate);
			jQuery('#releasedate-btn').find('span').text(formattedDate).removeClass('text-gray-400').addClass('text-gray-900 font-medium');
		}
	};
}

if (typeof vrcLocationWopening !== 'undefined') {
	var originalVrcLocationWopening = vrcLocationWopening;
	vrcLocationWopening = function(mode) {
		// Call original function
		originalVrcLocationWopening(mode);
		
		// Update time selectors styling
		var timeSelect = mode === 'pickup' ? '#vrccomselph' : '#vrccomseldh';
		jQuery(timeSelect).parent().addClass('border-[#FE5001] ring-2 ring-[#FE5001]/12');
		setTimeout(function() {
			jQuery(timeSelect).parent().removeClass('border-[#FE5001] ring-2 ring-[#FE5001]/12');
		}, 1000);
	};
}

/* Handle calendar day clicks for new form */
jQuery(document.body).on('click', '.vrc-cdetails-cal-pickday', function() {
	var tdday = jQuery(this).attr('data-daydate');
	if (!tdday || !tdday.length) return;
	
	// Set date on hidden field and update button
	jQuery('#pickupdate').datepicker('setDate', tdday);
	jQuery('#pickupdate-btn').find('span').text(tdday).removeClass('text-gray-400').addClass('text-gray-900 font-medium');
	
	// Scroll to booking section and trigger logic
