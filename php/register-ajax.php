...
// From templates/rent/php/ → templates/rent/ → templates/ → Joomla root
$jbase = dirname(dirname(dirname(dirname(__FILE__))));
if (!file_exists($jbase . '/includes/defines.php')) {
	echo json_encode(['error' => 'Joomla root not found at: ' . $jbase]);
	exit;
}
...
try {
	$db = JFactory::getDbo();
	$db->setQuery('SELECT id FROM ' . $db->quoteName('#__users')
